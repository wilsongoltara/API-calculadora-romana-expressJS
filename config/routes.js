...
routes.get('/calculadora/soma/', (req, res) => {
    ...
    res.send({"soma": soma,
            "soma-romana:":converteRomano(soma)
    });
})
...
routes.get('/calculadora/numeros', (req, res) => {
    ...
    for(i in body)
    {
      algarismos.push(converteRomano(body[i]))
    }
...
routes.get('/calculadora/operacao/soma', (req, res) => {
    const body = req.body;
    let algarismos = [], i;
    let soma = converteRomano(somaRomano(body));
    let result = ``;
